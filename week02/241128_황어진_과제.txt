...
//            l l l l l
		...
		// 문자열을 입력받을 변수 선언
		// 문자열에서 세번째 문자 추출
		// 이중 반복문을 사용해서 5 * 5 사각형 출력
		// 반복문 안에서 추출한 문자 출력.

//		Scanner scan = new Scanner(System.in); // 입력 메소드 임포트
//		
//		String inputValue = "";					// 입력값을 받을 String 변수
//		char thing = ' ';						// 세번쨰 문자를 담을 변수
//		
//		System.out.print("세글자 이상의 문자열을 입력해주세요 : "); // 안내문 출력
//		inputValue = scan.nextLine();						// 입력값을 변수에 할당
//		thing = inputValue.charAt(2);						// 문자 하나를 추출해서 변수에 할당
//		for (int i = 1; i < 6; i++) { 					// 반복문 : 시작값은 1이고 6미만까지 시행하며, 각 시행마다 1씩 증가한다.
//			for (int j = 1; j <6; j++) {				// 반복문 : 시작값은 1이고 6미만까지 시행하며, 각 시행마다 1씩 증가한다.
//				
//				System.out.print(thing);				// 추출한 세번째 글자를 출력
...
//            l l l l l
		...
		// 이중 반복문을 사용해서 5 * 5 사각형 출력
		// 반복문 안에서 추출한 문자 출력.
		// 조건식으로 첫번째 행, 첫번째 열, 마지막 행, 마지막 열만 세번째 문자 출력하고 나머지는 공백 출력
...
//		for (int i = 1; i < 6; i++) { 						// 반복문 : 시작값은 1이고 6미만까지 시행하며, 각 시행마다 1씩 증가한다.
//			for (int j = 1; j <6; j++) {					// 반복문 : 시작값은 1이고 6미만까지 시행하며, 각 시행마다 1씩 증가한다.
//				if( (i==1 || i==5) ||(j==1 || j==5) ) {		//첫번째 행이거나 마지막 행일때, 첫번째 열이거나 마지막 열일때
//					System.out.print(thing + " ");			// 추출한 알파벳 출력
//				}else {										// 위 조건에 해당하지 않으면
//					System.out.print(" ");					// 공백 출력
//					System.out.print(" ");					// 공백 출력
//				}
...
//
		...
		// 각 등급의 학점을 double로 실수타입 변수로 선언
		// 세 과목의 입력값을 저장할 변수 선언
		// 입력값을 받아서 변수에 할당 * 띄어쓰기로 입력값 구분
		// 학점 연산 후
		// 학점에 부합하는 등급을 출력

//		Scanner scan = new Scanner(System.in); // scan 입력 메소드
//
//		int subject1 = 0; // 입력값 변수 선언
//		int subject2 = 0; // 입력값 변수 선언
//		int subject3 = 0; // 입력값 변수 선언
//		int avr = 0; // 평균 연산값 변수 선언
//
//		System.out.print("3과목의 성적을 입력해 주세요 : ");// 안내문 출력
//		subject1 = scan.nextInt(); // 입력값 변수 할당
//		subject2 = scan.nextInt(); // 입력값 변수 할당
//		subject3 = scan.nextInt(); // 입력값 변수 할당
//
//		avr = (subject1 + subject2 + subject3) / 3; // 평균값 계산후 변수에 할당
//		
//		System.out.println("과목의 평균 값 : "  + avr);
//
//		if (avr >= 95) {	// 평균값이 95보다 크거나 같을때
//			System.out.print("평균 학점 :4.5\n평균 성적 : A+");// 조건에 따라 학점과 등급 출력
//
//		} else if (avr >=90) {// 평균값이 90보다 크거나 같을때
//			System.out.printf("평균 학점 : 4.0\n평균 성적 : A");// 조건에 따라 학점과 등급 출력
//
//		} else if (avr >= 85) {// 평균값이 85보다 크거나 같을때
//			System.out.printf("평균 학점 : 3.5\n평균 성적 : B+");// 조건에 따라 학점과 등급 출력
//
//		} else if (avr >= 80) {// 평균값이 80와 크거나 같을때
//			System.out.printf("평균 학점 : 3.0\n평균 성적 : B");// 조건에 따라 학점과 등급 출력
//
//		} else if (avr >= 75) {// 평균값이 75보다 크거나 같을때
//			System.out.printf("평균 학점 : 2.5\n평균 성적 : C+");// 조건에 따라 학점과 등급 출력
//
//		} else if (avr < 75) {// 평균값이 75 미만일때
//			System.out.printf("평균 학점 : 0.0\n평균 성적 : F");// 조건에 따라 학점과 등급 출력
//
//		} else {// 정상 값이 아닌 경우
//			System.out.printf("잘못된 값을 입력하셨습니다. 올바른 값을 다시 입력해주세요");
//
//		}
//
	}
